...
function badge_categoria(string $cat): string {
    $map = [
        'Papel'    => '#f0fdf4;color:#166534',
        'Plástico' => '#eff6ff;color:#1d4ed8',
        'Vidrio'   => '#eef2ff;color:#4338ca',
        'Metal'    => '#fff7ed;color:#c2410c',
    ];
            ...
            <form method="POST" action="index.php?menu=material&opc=tabla" id="formMaterial">
                ...
                <div class="mat-form-grid">
                    ...
                    <div class="form-row">
                        ...
                        <input type="text" name="categoria" id="mat_categoria"
                               placeholder="Ej: Plástico" required
...
function validarFormMaterial() {
    const campos = [
        { id: 'mat_nombre',    errorId: 'err-mat-nombre' },
        { id: 'mat_categoria', errorId: 'err-mat-categoria' },
    ];
    let valido = true;
    for (const c of campos) {
        ...
        if (!valor || !SOLO_LETRAS.test(valor)) {
            input.classList.add('input-error');
            document.getElementById(c.errorId).classList.add('visible');
...
document.getElementById('confirmBtnOk').addEventListener('click', () => {
    const cb = _confirmCallback; // guarda antes de cerrar
    cerrarConfirm();
    if (cb) cb();
});
...
function limpiarErrores() {
    ['mat_nombre','mat_categoria'].forEach(id => {
...
function editarMaterial(id, nombre, categoria, unidad, precio, stock, stock_min) {
    ...
    document.getElementById('mat_categoria').value = categoria;
